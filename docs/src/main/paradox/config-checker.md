...
# Pekko Config Checker

Pekko comes with a massive amount of configuration settings that can be tweaked.
...
```
[WARN] [10/01/2015 18:25:03.107] [main] [pekko.diagnostics.ConfigChecker] Configuration recommendation: Use throughput-deadline-time when dispatcher is configured with high throughput [200] batching to avoid unfair processing. Related config properties: [my-dispatcher.throughput = 200, my-dispatcher.throughput-deadline-time]. You may disable this check by adding [dispatcher-throughput] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

The log messages are emitted by the `pekko.diagnostics.ConfigChecker` logger, which is good to
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #typo }

That would result in these warnings:

```
pekko.log-level is not an Pekko configuration setting. Is it a typo or is it placed in the wrong section? Application specific properties should be placed outside the "pekko" config tree. Related config properties: [pekko.log-level]. You may disable this check by adding [typo] to configuration string list pekko.diagnostics.checker.disabled-checks.

pekko.default-dispatcher.throughput is not an Pekko configuration setting. Is it a typo or is it placed in the wrong section? Application specific properties should be placed outside the "pekko" config tree. Related config properties: [pekko.default-dispatcher.throughput]. You may disable this check by adding [typo] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
exceptions to sections that are supposed to be added to (e.g. `pekko.actor.serializers`).

It is only checking for typos in configuration paths starting with "pekko".

Application specific properties that are not defined by Pekko should be placed outside
the "pekko" config tree, but if you still have to define such a configuration property
inside "pekko" you can confirm that it is not a typo or misplacement by adding the
configuration path to `pekko.diagnostics.checker.confirmed-typos`. All properties starting
...
```
pekko.diagnostics.checker.confirmed-typos = [
  pekko.myapp.call-timeout,
  pekko.some-library
]
```
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #power-user }

The warning would look like:

```
pekko.cluster.gossip-interval is an advanced configuration setting. Make sure that you fully understand the implications of changing the default value. You can confirm that you know the meaning of this configuration setting by adding [pekko.cluster.gossip-interval] to configuration string list pekko.diagnostics.checker.confirmed-power-user-settings. Related config properties: [pekko.cluster.gossip-interval = 5s]. Corresponding default values: [pekko.cluster.gossip-interval = 1s]. You may disable this check by adding [power-user-settings] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #dispatcher-throughput }
...
When using Pekko Cluster this is the important failure detector, and you should normally not
worry about the other two failure detectors. Each node in an Pekko Cluster monitors a few
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #cluster-fd-short }

```
Cluster failure detector acceptable-heartbeat-pause of [1000 ms] is probably too short to be meaningful. It may cause marking nodes unreachable and then back to reachable because of false failure detection caused by for example GC pauses. Related config properties: [pekko.cluster.failure-detector.acceptable-heartbeat-pause = 1s]. Corresponding default values: [pekko.cluster.failure-detector.acceptable-heartbeat-pause = 3 s]. You may disable this check by adding [cluster-failure-detector] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #cluster-fd-ratio }

```
Cluster failure detector ratio [2] between acceptable-heartbeat-pause and heartbeat-interval is too small, decrease the heartbeat-interval and/or increase acceptable-heartbeat-pause. Otherwise it may trigger false failure detection and resulting in quarantining of remote system. Related config properties: [pekko.cluster.failure-detector.acceptable-heartbeat-pause = 6s, pekko.cluster.failure-detector.heartbeat-interval = 3s]. Corresponding default values: [pekko.cluster.failure-detector.acceptable-heartbeat-pause = 3 s, pekko.cluster.failure-detector.heartbeat-interval = 1 s]. You may disable this check by adding [cluster-failure-detector] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
 ...
 * sane relation between `heartbeat-interval` and `pekko.cluster.unreachable-nodes-reaper-interval`

### Remote Watch Failure Detector

In case you are not using Pekko Cluster but plain Pekko Remoting the remote watch failure detector is used
for `watch` between actors running on different nodes.

Note that it is not used for `watch` between actors that are running on nodes in the same Pekko Cluster,
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #remote-watch-fd-short }

```
Remote watch failure detector acceptable-heartbeat-pause of [3000 ms] is probably too short to be meaningful. It may cause quarantining of remote system because of false failure detection caused by for example GC pauses. Related config properties: [pekko.remote.watch-failure-detector.acceptable-heartbeat-pause = 3s]. Corresponding default values: [pekko.remote.watch-failure-detector.acceptable-heartbeat-pause = 10 s]. You may disable this check by adding [remote-watch-failure-detector] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
In case you are using Pekko Cluster is not recommended modifying the defaults remote watch failure detector. 

```
Remote watch failure detector shouldn't be changed when cluster is used. Related config properties: [pekko.remote.watch-failure-detector.*]. You may disable this check by adding [remote-watch-failure-detector-with-cluster] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

## More pekko-actor checks

### actor-ref-provider

```
[some.Other] is not a supported ActorRef provider. Use one of [pekko.actor.LocalActorRefProvider, pekko.remote.RemoteActorRefProvider, pekko.cluster.ClusterActorRefProvider]. Related config properties: [pekko.actor.provider = some.Other]. Corresponding default values: [pekko.actor.provider = pekko.actor.LocalActorRefProvider]. You may disable this check by adding [actor-ref-provider] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

### jvm-exit-on-fatal-error

```
Don't use jvm-exit-on-fatal-error=off. It's safer to shutdown the JVM in case of a fatal error, such as OutOfMemoryError. Related config properties: [pekko.jvm-exit-on-fatal-error = off]. Corresponding default values: [pekko.jvm-exit-on-fatal-error = on]. You may disable this check by adding [jvm-exit-on-fatal-error] to configuration string list pekko.diagnostics.checker.disabled-checks.
```


## More pekko-remote checks


### enabled-transports

```
[pekko.remote.netty.udp] is not a recommended transport for remote actor messages in production. Related config properties: [pekko.remote.enabled-transports]. You may disable this check by adding [enabled-transports] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
```
You have configured maximum-frame-size to [2097152 bytes]. We recommend against sending too large messages, since that may cause other messages to be delayed. For example, it's important that failure detector heartbeat messages have a chance to get through without too long delays. Try to split up large messages into smaller chunks, or use another communication channel (HTTP, Pekko IO) for large payloads. Related config properties: [pekko.remote.artery.advanced.maximum-frame-size = 2MiB]. Corresponding default values: [pekko.remote.artery.advanced.maximum-frame-size = 128000b]. You may disable this check by adding [maximum-frame-size] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

### default-remote-dispatcher-size

```
Don't use too small pool size [1] for the default-remote-dispatcher-size. Related config properties: [pekko.remote.default-remote-dispatcher]. You may disable this check by adding [default-remote-dispatcher-size] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

### create-actor-remotely 

```
Deploying an actor remotely is deprecated and not supported. As per https://doc.pekko.io/docs/pekko/current/remoting.html#creating-actors-remotely Related config properties: [pekko.actor.deployment."/...".remote"]. You may disable this check by adding [create-actor-remotely] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

### remote-artery-disabled

```
Classic remoting is deprecated since Pekko 2.6.0 and will be removed in Pekko 2.8.0. Use Artery instead. Related config properties: [pekko.remote.artery.enabled = false]. Corresponding default values: [pekko.remote.artery.enabled = on]. You may disable this check by adding [remote-artery-disabled] to configuration string list pekko.diagnostics.checker.disabled-checks.
```

### remote-prefer-cluster

```
Some features, such as remote watch, will be unsafe when using remote without Pekko Cluster. Related config properties: [pekko.actor.provider = pekko.remote.RemoteActorRefProvider]. Corresponding default values: [pekko.actor.provider = local]. You may disable this check by adding [remote-prefer-cluster] to configuration string list pekko.diagnostics.checker.disabled-checks. 
```

## More pekko-cluster checks

### cluster-dispatcher

```
Normally it should not be necessary to configure a separate dispatcher for the Cluster. The default-dispatcher should be sufficient for performing the Cluster tasks, i.e. pekko.cluster.use-dispatcher should not be changed. If you have Cluster related problems when using the default-dispatcher that is typically an indication that you are running blocking or CPU intensive actors/tasks on the default-dispatcher. Use dedicated dispatchers for such actors/tasks instead of running them on the default-dispatcher, because that may starve system internal tasks. Related config properties: [pekko.cluster.use-dispatcher = disp1]. Corresponding default values: [pekko.cluster.use-dispatcher = ]. You may disable this check by adding [cluster-dispatcher] to configuration string list pekko.diagnostics.checker.disabled-checks.
```
...
@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #disabled-checks }

It is also possible to disable all checks with:

@@snip [ConfigCheckerSpec.scala](/diagnostics/src/test/scala/org/apache/pekko/diagnostics/ConfigCheckerSpec.scala) { #disabled }
